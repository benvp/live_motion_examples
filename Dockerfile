...
RUN apt-get update -y && apt-get install -y build-essential git curl bash curl \
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/live_motion_examples ./
